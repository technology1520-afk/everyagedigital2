...
export default function HomePage() {
  ...
  return (
    <div className="space-y-16 sm:space-y-24 pb-12">
      {/* 1. Hero Section */}
      <section className="pt-12 sm:pt-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center">
        <div className="max-w-3xl mx-auto space-y-5">
          ...
          <h1 className="font-serif text-4xl sm:text-6xl font-extrabold text-neutral-900 tracking-tight leading-[1.15]">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            <Link
              href="/shop"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[#1D438A] text-white text-xs sm:text-sm font-semibold hover:bg-[#153266] transition-colors shadow-sm"
            >
            ...
            <Link
              href="/assistant"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white border border-[#E2E5EB] text-neutral-800 text-xs sm:text-sm font-semibold hover:bg-neutral-50 transition-colors shadow-xs"
            >
              <Sparkles className="w-4 h-4 text-[#1D438A]" />
          ...
          <div className="pt-6 max-w-2xl mx-auto">
            <form action="/search" method="GET" className="relative flex items-center">
              ...
              <input
                type="text"
                name="q"
                placeholder="What are you looking for today? (e.g. ergonomic mouse, habit books...)"
                className="w-full pl-12 pr-28 py-3.5 bg-white border border-[#E2E5EB] rounded-2xl text-xs sm:text-sm text-neutral-900 placeholder:text-neutral-400 focus:outline-hidden focus:border-[#1D438A] shadow-xs"
              />
              <button
                type="submit"
                className="absolute right-2.5 px-4 py-2 bg-neutral-900 text-white text-xs font-semibold rounded-xl hover:bg-neutral-800 transition-colors cursor-pointer"
              >
                Search
              </button>
            </form>
            ...
            {/* Quick search chips */}
            <div className="flex flex-wrap items-center justify-center gap-1.5 mt-3 text-left">
              <span className="text-[11px] text-neutral-400">Popular:</span>
              {searchExamples.map(term => (
                <Link
                  key={term}
                  href={`/search?q=${encodeURIComponent(term)}`}
                  className="text-[11px] text-neutral-600 hover:text-[#1D438A] bg-white border border-neutral-200/80 rounded-full px-2.5 py-0.5 hover:border-[#1D438A]/40 transition-colors"
                >
                  {term}
                </Link>
              ))}
      ...
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="border-t border-b border-[#E2E5EB] py-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {categories.map(cat => (
            <Link
              key={cat.slug}
              href={`/category/${cat.slug}`}
              className="p-3 bg-white rounded-xl border border-neutral-200/80 hover:border-[#1D438A]/50 transition-colors flex flex-col justify-between group shadow-xs"
            >
              <span className="text-xs font-bold text-neutral-800 group-hover:text-[#1D438A] transition-colors">
                {cat.name}
              </span>
      ...
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-8">
          ...
          <Link
            ...
            className="text-xs font-semibold text-[#1D438A] hover:underline flex items-center gap-1"
          >
      ...
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="bg-[#151515] text-white rounded-3xl p-8 sm:p-12 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8 shadow-xl">
          <div className="max-w-xl space-y-4">
            ...
            <div className="pt-2">
              <Link
                href="/assistant"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#1D438A] text-white text-xs sm:text-sm font-semibold rounded-xl hover:bg-[#234F9E] transition-colors"
              >
      ...
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="bg-[#F2EBDD]/50 border border-[#E0D3BC] rounded-3xl p-8 sm:p-10">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
            <div>
              <span className="text-xs font-mono uppercase tracking-widest text-neutral-600 font-semibold">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {ownedProducts.map(prod => (
              <div
                key={prod.id}
                className="bg-white border border-[#E2E5EB] rounded-2xl p-6 flex flex-col justify-between hover:shadow-md transition-shadow"
              >
                <div>
                  ...
                  <h3 className="text-lg font-bold text-neutral-900">
                    <Link href={`/shop/own-products/${prod.slug}`} className="hover:underline">
                      {prod.title}
                    </Link>
                  </h3>
                ...
                <div className="mt-6 pt-4 border-t border-neutral-100 flex items-center justify-between">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-1.5 py-2 px-4 rounded-lg text-xs font-semibold bg-[#1D438A] text-white hover:bg-[#153266] transition-colors"
                  >
                    <span>View Guide & Demo Checkout</span>
                    <ArrowRight className="w-3.5 h-3.5" />
                  </Link>
                </div>
              </div>
      ...
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="border border-[#E2E5EB] bg-white rounded-3xl p-8 sm:p-12">
