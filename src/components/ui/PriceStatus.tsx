...
export function PriceStatus({ offer, freshness, size = 'md', className = '' }: PriceStatusProps) {
  if (!offer) {
    return <span className={`text-neutral-400 text-sm italic ${className}`}>Price on request</span>;
  }
  ...
  if (freshness?.isStale) {
    return (
      <div className={`flex flex-col ${className}`}>
        ...
        <span className="text-[10px] text-neutral-400 mt-0.5">Price check overdue</span>
      </div>
    );
  }
  ...
  return (
    <div className={`flex items-baseline gap-2 ${className}`}>
      <span className={`${textSizes[size]} text-neutral-900`}>
